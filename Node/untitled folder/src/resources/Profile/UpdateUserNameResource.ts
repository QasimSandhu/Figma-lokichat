...
interface UpdateUserNameResourceProps {
...
class UpdateUserNameResourceResource extends ApiResponse {
  constructor(
    data: UpdateUserNameResourceProps | UpdateUserNameResourceProps[],
    ...
    if (!data) {
      ...
      const formattedData = Array.isArray(data)
        ? data.map(formatData)
        : formatData(data);
      super(ApiResponse.success(formattedData, "Username updated successfully", status));
    }
  }
}

function formatData(data: UpdateUserNameResourceProps): any {
  return {
    id: data._id,
    email: data.email,
...
export default UpdateUserNameResourceResource;
